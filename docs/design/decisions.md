...
The first durable D5 store is built on fjall 3 transactional keyspaces directly. No qualified fleet tier fits yet: pinax 0.0.4 has no multi-row transaction, no schema migration, and no encryption, and koina exposes no public content-addressed blob API. Per `kanon/crates/basanos/standards/STORAGE-TIERS.md`, this is a named migration exception: the owning crate declares pinax and koina as the target tiers in its roadmap, and removal of the direct fjall dependency is tracked in the STORAGE-TIERS exception issue dioptron#91. Retirement condition: pinax ships a multi-row transaction with schema migration and encryption, and koina exposes a public content-addressed blob API. Until both exist, raw fjall is the sanctioned substrate. Detail in `docs/design/custody-store.md`.
